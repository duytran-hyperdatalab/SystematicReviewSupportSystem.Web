...
**Retrieves a paginated and filtered list of candidate papers for a specific project.**
...
- **Query Parameters**:
  ...
  - `searchTerm` (string): Text search across Title, Authors, DOI, and Origin Paper Title.
...
interface CandidatePaperDto {
  ...
  originPaperTitle: string | null;  // NEW: Enriched metadata for UI
  originPaperAuthors: string | null; // NEW: Enriched metadata for UI
  title: string;
  authors: string | null;
